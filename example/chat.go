	...
	return h.Div(a.Attrs(
		...
		h.Div(a.Attrs(
			...
			h.P(a.Attrs(
				...
				h.Text("Real-time chat with broadcasting — messages sync instantly across all connected clients!"))),

		// Username setup
		h.Div(a.Attrs(
			a.Class("bg-gradient-to-r from-violet-50 to-purple-50 p-5 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
			h.Div(a.Attrs(
				a.Class("flex flex-col sm:flex-row items-stretch sm:items-end gap-3")),
				h.Div(a.Attrs(
					a.Class("flex-grow")),
					h.Label(a.Attrs(
						a.For("usernameInput"),
						a.Class("block text-sm font-semibold text-stone-700 mb-1")),
						h.Text("Your Display Name")),
					h.Input(a.Attrs(
						a.Type("text"),
						a.Id("usernameInput"),
		...
		h.Div(a.Attrs(
			a.Id("messages"),
			a.Class("bg-stone-100 p-4 rounded-xl border-2 border-stone-300 h-80 overflow-y-auto space-y-3")),
			func() []h.Element {
				if len(*chat.Messages) == 0 {
					return []h.Element{
						h.Div(a.Attrs(
							a.Class("flex items-center justify-center h-full text-stone-400")),
				...
				for _, msg := range *chat.Messages {
					...
					elements = append(elements, h.Div(a.Attrs(
						...
						h.Div(a.Attrs(
							a.Class(fmt.Sprintf("max-w-xs lg:max-w-md px-4 py-2.5 rounded-xl border-2 %s", bubbleClass))),
							h.Div(a.Attrs(
								a.Class("flex justify-between items-baseline mb-1 gap-3")),
								h.Span(a.Attrs(
									a.Class(fmt.Sprintf("font-bold text-xs %s", textClass))),
									h.Text(msg.User)),
								h.Span(a.Attrs(
									a.Class("text-xs text-stone-400"),
									a.Style(css.FontFamily("'JetBrains Mono', monospace"))),
									h.Text(msg.TimeStamp.Format("15:04")))),
							h.P(a.Attrs(
								a.Class(fmt.Sprintf("text-sm %s", textClass))),
								h.Text(msg.Text)))))
				}
				return elements
			}()...),
		...
		// Message input
		h.Div(a.Attrs(
			a.Class("bg-white p-4 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
			h.Div(a.Attrs(
				...
				h.Span(a.Attrs(
					...
					h.Text("Posting as")),
				h.Span(a.Attrs(
					a.Class("text-sm font-bold text-stone-900 bg-stone-100 px-2 py-0.5 rounded")),
					h.Text(func() string {
						if chat.Username == "" {
							return "Anonymous"
						}
