...
func (f Form) renderValues() h.Element {
	return h.Div(a.Attrs(
		a.Class("bg-gradient-to-br from-emerald-50 to-teal-50 p-6 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
		h.H2(a.Attrs(
			a.Class("text-lg font-bold text-stone-900 mb-4 flex items-center gap-2")),
			h.Span(a.Attrs(),
				h.Text("📊")),
			h.Text("Live Form State")),
		h.Div(a.Attrs(
			a.Class("space-y-3 text-sm"),
...
func formValueRow(key, value string) h.Element {
	...
	return h.Div(a.Attrs(
		...
		h.Span(a.Attrs(
			a.Class("text-emerald-600 font-semibold shrink-0")),
			h.Text(key+":")),
		h.Span(a.Attrs(
			a.Class("text-stone-700 break-all")),
			h.Text(displayValue)))
}
	...
	return h.Div(a.Attrs(
		...
		h.Div(a.Attrs(
			...
			h.P(a.Attrs(
				...
				h.Text("Real-time form state binding — watch the state update as you type!"))),
		renderExplanatoryNote(
			...
			`),
		h.Div(a.Attrs(
			a.Class("grid grid-cols-1 lg:grid-cols-2 gap-8")),
			// Form column
			h.Form(a.Attrs(
				a.Class("space-y-5 bg-white p-6 rounded-xl border-2 border-stone-900 shadow-brutal-sm"),
				a.Id("my-form")),
				h.H3(a.Attrs(
					a.Class("text-lg font-bold text-stone-900 mb-4")),
					h.Text("Input Fields")),

				formField("Text Input", h.Input(a.Attrs(
				...
				formField("Select", h.Select(a.Attrs(
					...
					func() []h.Element {
						...
						for _, option := range f.Options {
							options = append(options, h.Option(a.Attrs(
								a.Value(option),
								func() a.Attribute {
				...
				formField("Multiple Select", h.Select(a.Attrs(
					...
					h.Option(a.Attrs(
						a.Value("first"),
						f.isSelected("first", f.MultipleTextInput)),
						h.Text("first")),
					h.Option(a.Attrs(
						a.Value("second"),
						f.isSelected("second", f.MultipleTextInput)),
						h.Text("second")),
					h.Option(a.Attrs(
						a.Value("third"),
						f.isSelected("third", f.MultipleTextInput)),
						h.Text("third")),
					h.Option(a.Attrs(
						a.Value("fourth"),
						f.isSelected("fourth", f.MultipleTextInput)),
						h.Text("fourth")))),

				formField("Text Area", h.TextArea(a.Attrs(
					a.Class("mt-1 block w-full rounded-lg px-4 py-2.5 text-stone-900 placeholder-stone-400"),
					a.OnKeyUp(gt.BasicUpdateForm("FORM_UPDATE", "my-form")),
					a.Name("TextboxInput"),
					a.Rows(4),
				...
				h.Div(a.Attrs(
					a.Class("space-y-2")),
					h.Label(a.Attrs(
						a.Class("block text-sm font-semibold text-stone-700")),
						h.Text("Radio Selection")),
					h.Div(a.Attrs(
						a.Class("flex gap-4 mt-2")),
				...
				// Checkbox
				h.Div(a.Attrs(
					a.Class("flex items-center gap-3 p-3 bg-stone-50 rounded-lg")),
					h.Input(a.Attrs(
					...
					h.Label(a.Attrs(
						a.Class("font-medium text-stone-700")),
						h.Text("Toggle this checkbox")))),
...
func radioOption(label, value, name string, checked bool) h.Element {
	return h.Label(a.Attrs(
		...
		h.Span(a.Attrs(
			a.Class("text-sm font-medium text-stone-700")),
			h.Text(label)))
}
