...
func (m *Model) OnRouteChange(path string) {
	// Ridiculously simple routing model -
	...
	el := h.Html(a.Attrs(
		...
		h.Head(a.Attrs(),
			...
			h.Meta(a.Attrs(
				...
				a.Content("ie=edge"))),
			h.Title(a.Attrs(),
				h.Text(fmt.Sprintf("Gotea | %s", m.TemplateName))),
			h.Script(a.Attrs(
				a.Src("https://cdn.tailwindcss.com"))),
			h.Link(a.Attrs(
				a.Rel("preconnect"),
			...
			h.Link(a.Attrs(
				a.Rel("stylesheet"),
				a.Href("https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=JetBrains+Mono:wght@400;600&family=Outfit:wght@300;400;500;600;700&display=swap"))),
			h.Link(a.Attrs(
				a.Rel("icon"),
				a.Type("image/svg+xml"),
				a.Href("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>🍵</text></svg>"))),
			h.Style(a.Attrs(),
				h.Text(customStyles()))),
		h.Body(a.Attrs(
			a.Class("min-h-screen bg-stone-100"),
			a.Style(css.FontFamily("'Outfit', sans-serif"))),
			// Decorative background pattern
			h.Div(a.Attrs(
				a.Class("fixed inset-0 -z-10 overflow-hidden")),
				h.Div(a.Attrs(
					a.Class("absolute -top-40 -right-40 w-96 h-96 bg-amber-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"))),
				h.Div(a.Attrs(
					a.Class("absolute top-40 -left-20 w-72 h-72 bg-emerald-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000"))),
				h.Div(a.Attrs(
					a.Class("absolute -bottom-20 left-1/2 w-80 h-80 bg-rose-200 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-4000")))),
			// Header/Nav
			h.Header(a.Attrs(
				a.Class("sticky top-0 z-50 backdrop-blur-md bg-stone-100/80 border-b-2 border-stone-900")),
				h.Div(a.Attrs(
					a.Class("max-w-7xl mx-auto px-6 py-4")),
					h.Div(a.Attrs(
						...
						h.A(a.Attrs(
							...
							h.Span(a.Attrs(
								a.Class("text-4xl transition-transform group-hover:rotate-12")),
								h.Text("🍵")),
							h.Div(a.Attrs(),
								h.Span(a.Attrs(
									a.Class("block text-2xl font-bold tracking-tight text-stone-900"),
									a.Style(css.FontFamily("'DM Serif Display', serif"))),
									h.Text("Gotea")),
								h.Span(a.Attrs(
									a.Class("block text-xs font-medium text-stone-500 tracking-widest uppercase")),
									h.Text("Elm Architecture for Go")))),
						// Nav pills
						h.Nav(a.Attrs(
							a.Class("hidden lg:flex items-center gap-1")),
			...
			// Main content
			h.Main(a.Attrs(
				a.Class("max-w-7xl mx-auto px-6 py-12")),
				h.Div(a.Attrs(
					a.Class("bg-white border-2 border-stone-900 rounded-2xl shadow-brutal p-8 md:p-12")),
					h.Div(a.Attrs(
						a.Id("view")),
						h.UnsafeRaw(string(m.RenderRoute(m)))))),
			// Footer
			h.Div(a.Attrs(
				a.Class("mt-auto py-8"),
				a.Role("contentinfo")),
				h.Div(a.Attrs(
					a.Class("max-w-7xl mx-auto px-6")),
					h.Div(a.Attrs(
						a.Class("flex flex-col md:flex-row items-center justify-between gap-4 text-stone-500")),
						h.Div(a.Attrs(
							a.Class("flex items-center gap-2 text-sm")),
							h.Span(a.Attrs(
								a.Class("font-mono text-xs bg-stone-200 px-2 py-1 rounded")),
								h.Text(m.sessionID.String()[:8]+"...")),
							h.Span(a.Attrs(
								a.Class("text-stone-300")),
								h.Text("•")),
							h.Span(a.Attrs(),
								h.Text(m.Router.Route))),
						h.Div(a.Attrs(
							a.Class("text-sm")),
							h.Text("Built with "),
							h.Span(a.Attrs(
								a.Class("font-semibold text-stone-700")),
								h.Text("Gotea")),
...
func navPill(href, text string, active bool) h.Element {
	if active {
		return h.A(a.Attrs(
			a.Href(href),
			a.Class("px-4 py-2 text-sm font-semibold bg-stone-900 text-white rounded-full")),
			h.Text(text))
	}
	return h.A(a.Attrs(
		a.Href(href),
		a.Class("px-4 py-2 text-sm font-medium text-stone-600 hover:text-stone-900 hover:bg-stone-200 rounded-full transition-colors")),
		h.Text(text))
}
...
func renderHome(counter int) h.Element {
	return h.Div(a.Attrs(
		...
		// Hero section
		h.Div(a.Attrs(
			a.Class("text-center space-y-4 pb-8")),
			h.H1(a.Attrs(
				a.Class("text-5xl md:text-6xl font-bold text-stone-900 tracking-tight"),
				a.Style(css.FontFamily("'DM Serif Display', serif"))),
				h.Text("Welcome to Gotea")),
			h.P(a.Attrs(
				a.Class("text-xl text-stone-600 max-w-2xl mx-auto leading-relaxed")),
			...
			// Feature badges
			h.Div(a.Attrs(
				a.Class("flex flex-wrap justify-center gap-3 pt-4")),
		...
		// Counter demo
		h.Div(a.Attrs(
			a.Class("bg-gradient-to-br from-stone-50 to-stone-100 border-2 border-stone-900 rounded-2xl p-8 shadow-brutal-sm")),
			h.Div(a.Attrs(
				a.Class("flex flex-col md:flex-row md:items-center md:justify-between gap-6")),
				h.Div(a.Attrs(
					a.Class("space-y-2")),
					h.H2(a.Attrs(
						a.Class("text-2xl font-bold text-stone-900")),
		...
		h.Div(a.Attrs(
			a.Class("space-y-6")),
			h.H3(a.Attrs(
				a.Class("text-lg font-semibold text-stone-900")),
				h.Text("Explore the demos →")),
			h.Div(a.Attrs(
				a.Class("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4")),
		...
		// Divider
		h.Div(a.Attrs(
			a.Class("border-t-2 border-dashed border-stone-300 my-4"))),
		// Error handling demo
		h.Div(a.Attrs(
			a.Class("bg-rose-50 border-2 border-rose-300 rounded-xl p-6")),
			h.H3(a.Attrs(
				a.Class("text-lg font-semibold text-rose-900 mb-2")),
				h.Text("🚨 Error Handling Demo")),
			h.P(a.Attrs(
				a.Class("text-rose-700 text-sm mb-4")),
				h.Text("Click to trigger a panic and see how Gotea handles server-side errors gracefully.")),
			renderCrash()))
}
...
func featureBadge(icon, label, color string) h.Element {
	...
	return h.Span(a.Attrs(
		a.Class(fmt.Sprintf("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border %s", colorClasses[color]))),
		h.Span(a.Attrs(),
			h.Text(icon)),
		h.Text(label))
}
...
func demoCard(href, icon, title, description string) h.Element {
	return h.A(a.Attrs(
		...
		h.Div(a.Attrs(
			...
			h.Div(a.Attrs(),
				h.H4(a.Attrs(
					a.Class("font-semibold text-stone-900 group-hover:text-emerald-700 transition-colors")),
					h.Text(title)),
				h.P(a.Attrs(
					a.Class("text-sm text-stone-500 mt-1")),
					h.Text(description)))))
}
	...
	return h.Div(a.Attrs(
		...
		h.Div(a.Attrs(
			...
			h.H1(a.Attrs(
				...
				a.Style(css.FontFamily("'DM Serif Display', serif"))),
				h.Text("🧭 Client-Side Routing")),
			h.P(a.Attrs(
				a.Class("text-stone-600")),
				h.Text("SPA-style navigation without page reloads, powered by WebSocket messages."))),

		// Current route display
		h.Div(a.Attrs(
			a.Class("bg-gradient-to-r from-sky-50 to-blue-50 p-5 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
			h.Div(a.Attrs(
				a.Class("flex items-center justify-between")),
				h.Div(a.Attrs(),
					h.Span(a.Attrs(
						a.Class("text-sm font-medium text-stone-500")),
						h.Text("Current Route")),
					h.Div(a.Attrs(
						a.Class("text-2xl font-bold text-stone-900"),
		...
		// Link examples
		h.Div(a.Attrs(
			a.Class("grid grid-cols-1 md:grid-cols-2 gap-4")),
			// Internal link
			h.Div(a.Attrs(
				a.Class("bg-white p-6 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
				h.H3(a.Attrs(
					a.Class("text-sm font-semibold text-stone-500 uppercase tracking-wide mb-3")),
					h.Text("Internal Link")),
				h.A(a.Attrs(
				...
				h.P(a.Attrs(
					a.Class("text-sm text-stone-500 mt-2")),
					h.Text("Handled by Gotea (no page reload)"))),
			// External link
			h.Div(a.Attrs(
				a.Class("bg-white p-6 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
				h.H3(a.Attrs(
					a.Class("text-sm font-semibold text-stone-500 uppercase tracking-wide mb-3")),
					h.Text("External Link")),
				h.A(a.Attrs(
				...
				h.P(a.Attrs(
					a.Class("text-sm text-stone-500 mt-2")),
					h.Text("Normal browser navigation")))),

		// Route parameters
		h.Div(a.Attrs(
			a.Class("bg-white p-6 rounded-xl border-2 border-stone-900 shadow-brutal-sm")),
			h.H3(a.Attrs(
				a.Class("text-lg font-bold text-stone-900 mb-4")),
				h.Text("Query Parameters")),
			h.Div(a.Attrs(
				a.Class("flex flex-wrap gap-2 mb-4")),
			...
			h.Div(a.Attrs(
				a.Class("bg-stone-100 p-4 rounded-lg flex items-center gap-3")),
				h.Span(a.Attrs(
					a.Class("text-sm text-stone-500")),
					h.Text("myparam =")),
				h.Span(a.Attrs(
					a.Class("text-lg font-bold text-stone-900"),
					a.Style(css.FontFamily("'JetBrains Mono', monospace"))),
					h.Text(func() string {
		...
		// Route data
		h.Div(a.Attrs(
			a.Class("bg-gradient-to-r from-amber-50 to-yellow-50 p-5 rounded-xl border-2 border-amber-300")),
			h.Div(a.Attrs(
				...
				h.Div(a.Attrs(),
					h.H4(a.Attrs(
						a.Class("font-semibold text-amber-900")),
						h.Text("Route-Specific Data")),
					h.P(a.Attrs(
						a.Class("text-sm text-amber-800")),
						h.Text(fmt.Sprintf("Random number generated in OnRouteChange hook: %d", m.RouteData)))))),
...
func renderExplanatoryNote(title, content string) h.Element {
	return h.Details(a.Attrs(
		a.Class("bg-amber-50 border-2 border-stone-900 rounded-xl mb-8 shadow-brutal-sm overflow-hidden")),
		h.Summary(a.Attrs(
